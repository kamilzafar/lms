<template>
	<div>
		...
		<div v-if="courses?.data?.length" class="mt-10">
		...
		<div
			v-if="!courses?.data?.length && !liveClasses?.data?.length && !courses?.loading && !liveClasses?.loading"
			class="flex flex-col items-center justify-center mt-60"
		>
			<GraduationCap class="size-10 mx-auto stroke-1 text-ink-gray-5" />
			<div class="text-lg font-semibold text-ink-gray-7 mb-1.5">
				{{ isAdmin ? __('No courses created') : __('No courses enrolled') }}
			</div>
			<div
			...
			>
				{{
					isAdmin
						...
						: __('You are not enrolled in any courses yet. Browse our courses to get started!')
				}}
			</div>
			<router-link
				v-if="isAdmin"
		...
		<!-- Upcoming Evaluations for all roles -->
		<div class="mt-10">
			<UpcomingEvaluations :forHome="true" />
		</div>

		<!-- Upcoming Evaluations (Admin only) -->
		<div v-if="isAdmin && evals?.data?.length" class="mt-10">
			<div class="font-semibold text-lg text-ink-gray-9 mb-3">
				{{ __('Upcoming Evaluations to Conduct') }}
			</div>
...
const props = defineProps<{
	...
	courses?: { data?: any[], loading?: boolean }
	evals?: { data?: any[] }
	isAdmin: boolean
}>()
...
const courseSectionTitle = computed(() => {
	if (props.isAdmin) {
		return __('Courses Created')
	}
	// For students/teachers, check if enrolled or popular
	if (props.courses?.data?.[0]?.membership) {
		return __('My Courses')
	}
	return __('Our Popular Courses')
})
...
// Teachers CAN start classes, but NOT create/delete
const canStartClass = () => {
	return (
